...
CLOUD-004 (2026-09-15): skills list unchanged (no install/uninstall). Skill Selection Gate re-run; loaded during the task: `supabase`, `supabase-postgres-best-practices`, `security-guidance`, `securability-engineering`, `vitest`, `gh-cli`, `github` (`mcp-server-review` not needed — no config change). Not applied (unchanged): `semgrep`/`codeql` (CLI unavailable on host — recorded limitation; structural migration invariants + DB/RLS assertion suites + MCP advisors substituted).
CLOUD-006 (2026-09-15): skills list unchanged (no install/uninstall). Skill Selection Gate re-run; loaded during the task: `supabase`, `supabase-postgres-best-practices`, `security-guidance`, `vitest`, `gh-cli`, `github` (`securability-engineering` not separately loaded — DDL+trigger-only change, no service-layer code; `mcp-server-review` not needed — no config change). Not applied (unchanged): `semgrep`/`codeql` (CLI unavailable on host — recorded limitation; structural migration invariants + DB/RLS assertion suites + MCP advisors substituted).

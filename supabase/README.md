...
## Admin user directory (CLOUD-013)

`20260916120000_establish_admin_user_directory.sql` adds the final PRD §7 admin-dashboard query surface, the searchable user directory. See ADR-025 for the full decision; highlights:

- **Server-owned, immutable public user ID**: `public.profiles.public_user_id text NOT NULL UNIQUE CHECK (CHAR_LENGTH BETWEEN 8 AND 200)` value `'user-' || replace(gen_random_uuid()::text, '-', '')` (37-char lowercase hex, UUIDv4 entropy). Client values are overwritten by `BEFORE INSERT` trigger `profiles_set_public_user_id`; `BEFORE UPDATE` trigger `profiles_guard_public_user_id_immutable` raises `CLOUD-013: public_user_id is immutable` for every session including postgres. Both are SECURITY INVOKER, `search_path = pg_catalog`, EXECUTE revoked from every app role. The migration backfills before `SET NOT NULL`; internal UUIDs (`auth.users.id`/`profiles.id`) never leave the server.
- **One new SECURITY DEFINER RPC** `admin_users(p_search text, p_offset integer)` → `jsonb`, the third justified exception (ADR-010 escape hatch, ADR-025, same shape as ADR-016): body gates FIRST on `auth.uid()` + stored `profiles.role = 'admin'` (`CLOUD-013: admin access required`, claims tampering grants nothing); EXECUTE revoked from `public`/`anon`/`service_role`, granted to `authenticated` only; `search_path = pg_catalog`, all schema-qualified, no dynamic SQL.
- **Bounded deterministic query**: `ORDER BY lower(u.email) COLLATE "C", u.id`; page size 25 (`limit 26 + offset`, slice → `has_more`); offset `0..100000` (out of range raises `CLOUD-013`); search is literal case-insensitive substring (`position(lower(v) in lower(col)) > 0`) over email/public_user_id/display_name, trimmed, `''`→NULL, truncated to 100 chars.
- **Minimal closed projection**: envelope `{users, has_more, page_size, offset, search, generated_at}`; per user exactly 11 keys (`public_user_id, email, display_name, role, account_status, created_at, updated_at, last_sign_in_at, entitlement, devices, sessions`). `entitlement` = newest soravo row `{plan, status, starts_at, expires_at}` (JSON null when none); `devices`/`sessions` = `{count, active, platforms|last_seen_at}` via `LEFT JOIN LATERAL` correlated aggregates. No provider/payment refs, passwords, raw identity metadata, or internal ids. No new table grants, policies, or write paths.

Verification: `db_assertions.sql` grows to 60 checks — check 16 SECURITY DEFINER whitelist is now exactly four functions (the ADR-016 metrics trio + `admin_users`); new checks 57–60 pin the column and both triggers; `rls_assertions.sql` scenarios U1–U21 (privilege denials, in-body gate denials incl. claim-tampered and postgres-without-claims, admin allow with exact 11-key projection, case-insensitive search, bounded search/offset, 25/page pagination with `has_more` reconstructing the ORDER BY without gaps/dupes, byte-level determinism, key-set minimization, summary exactness, account-status derivation, `public_user_id` server ownership + immutability); `migration-guard.test.mjs` unchanged 12/12. All green live on dev project `zbzhlhoxblguepplqppw`. Security advisor: the intentional ADR-016/025 SECURITY DEFINER WARN now covers four functions (metrics trio + `admin_users`); performance advisor unchanged (1 pre-existing INFO). The WEB-009 admin directory frontend wiring (search box consuming this RPC) remains an explicit future task; the dashboard honestly reports the directory as not yet wired.
